...
Other printable characters replace the selection. Without a selection, `^`, `_`, and `/` immediately consume the preceding simple operand or adjacent structure. For example, type `x^2`, `x_2`, or `ab/2` without accepting a suggestion. In `a+b/2`, only `b` becomes the numerator. Use a selection to include `a+b` together. `x^2_3` and `x_3^2` create both scripts on the same base. A leading `/` starts in the empty numerator, while leading `^` or `_` starts in the empty script. Without a selection, `(`, `[`, `{` and `|` create an empty fence at the caret. At the end of its immediate body, the matching closing key moves outside the fence. `<` and `>` remain comparisons. Use `cases` or the toolbar for cases. See the [keyboard reference](KEYBOARD.md#type-brackets-directly) for norm, angle and half-open fences. Literal text slots retain text input. Ctrl/Cmd/Alt combinations and IME composition are not structural shortcuts. Multi-line selections remain selected and unchanged when a wrapping key is pressed. Rectangular matrix-cell selection retains its own typing behavior.
